...
uint32_t ST7735_DrawString(uint16_t x, uint16_t y, char *pt, int16_t textColor, int16_t backColor, int16_t size){
  uint32_t count = 0;
  while(*pt){
    if(*pt == ' ')
    {
        ST7735_DrawCharS(x, y, *pt, textColor, backColor, 1);
        x = x + (3*size);
    }
    else
    {
        ST7735_DrawCharS(x, y, *pt, textColor, backColor, size);
        x = x + (6*size);
    }
    pt++;
    count++;
...
void ST7735_OutUDec(uint32_t n){
  ...
  ST7735_DrawString(StX,StY,Message,StTextColor, ST7735_BLACK, 1);
...
void ST7735_OutChar(char ch){
  if((ch == 10) || (ch == 13) || (ch == 27)){
    ...
    ST7735_DrawString(0,StY,"                     ",StTextColor, ST7735_BLACK, 1);
    return;
  }
